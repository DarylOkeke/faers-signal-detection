...
def validate_views(conn):
    ...
    df = pd.read_sql_query(
        "SELECT name FROM sqlite_master WHERE type='view' AND name LIKE 'v_%2023%' ORDER BY name;",
        conn
    )
...
def main():
    ...
    try:
        ...
        print("=" * 80)
        print("Next:")
        print(" 1) python -m src.compute_summary --db data/faers+medicare.db --view v_signal_2x2_2023 --out results/tables/summary_2023_ps.csv")
